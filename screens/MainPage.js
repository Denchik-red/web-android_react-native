...
function MainPage() {
    ...
    return (
        ...
        >
            ...
            {/* Список подписок */}
            <Text className={`text-lg font-semibold mb-4 ${isDark ? 'text-zinc-100' : 'text-zinc-900'}`}>
                Активные подписки
            </Text>
            
            <View className="flex flex-row flex-wrap gap-4 mb-10">
                {subscribes.map((sub, index) => (
                    <View key={index} className="w-full sm:w-[48%] lg:w-[32%]">
                        <Subscribe
                            nom={sub.nom}
                            name={sub.name}
